...
def GA_reduce_gate(planes,gatesList,max_iter=500):
    #生成300随机数列
    generationList =[]
    # 保存每次迭代得到的最优解
    optimalSolutions = []
    ...
    for i in range(300):
        serise_pre = previlege[(i % gatesList.__len__()):(i % gatesList.__len__()) + planes.__len__()]
        a = []
        for ser_i in range( gatesList.__len__() ):
            a[ser_i] = serise_pre[ser_i] % gatesList
    ...
    for i in range(max_iter):
        ...
        #plot出来看看
        plt.scatter( i,np.min(list(evaluates)))
...
def GA_gates_Num(plane_order,planes,gatesList):
    ...
    for i in range(planes.__len__()):
        if not time_gate.__contains__(plane_order[i]):
            time_gate[plane_order[i]] = planes[i][6]*24*60+planes[i]+ 45
        else:
            if planes[i][1]*24*60+planes[i][2]<time_gate[plane_order[i]]:
                count = count + 1
            else:
                time_gate[plane_order[i]] = planes[i][6]*24*60+planes[i]+ 45
    return max(planes.__len__()-count,0.0000001)
